...
- **Memory.** `tensorcad-runtime measure` runs AdamW steps on the GPU and reads the allocator. At rest, weights, gradients and optimizer state match the analysis's 16 bytes a parameter to 2%. The activations saved with the whole model in bf16 match to 6%, for GPT-2 small and a Llama-style 150M model. Plain autocast saves 1.26–1.55× more: fp32 residuals and norms, plus a bf16 copy of every weight for the whole step, which the analysis does not yet model. `python.test.ts` holds the first two on a CUDA machine and skips without one.
