...
| Editor UI | **Reworked against CAD convention.** Orthogonal wires, a grid with snap, schematic-style blocks, a model tree with locking, typed pins, a status bar, named refusals, a definition editor, a selectable tensor, a feature timeline, an SVG export of the sheet and a volume view whose stages are named. Thirty passes, each with what was wrong and what replaced it, in `docs/explanation/interaction-design.md`. Every block and wire has a spoken name and the hierarchy can be walked by keyboard — Enter into a block, Escape back out onto it — and `bun run test:browser` drives the built editor in headless Chrome with real key events, in CI, because the unit suite has no browser to see what React Flow does with a key first. |
...
| M15 Memory, measured | **Phases 1–2 done.** `tensorcad-runtime measure` runs real training steps on the GPU and reads the allocator: the resting state matches the analysis to 2% and bf16 activations to 6%. Plain autocast, which saves 1.26–1.55× more, is now a recipe the analysis models to within 5%. Phase 3 is the peak, not the sum. |
...
### M14 — Your first design · Done

M13 made the editor legible to somebody who has just arrived. The next wall is
making something: File ▸ New gave an empty sheet and fifty-five blocks to wire
by hand, and a preset is somebody else's model at somebody else's size. The
engine could already scale a design to a size, both ways, and nothing offered it
as a way to start.

1. **New design, with two questions.** A kind of model and a size, or the one
   GPU its training has to fit on; the kind's reference design scaled to it.
   *Done:* `start.New` in the engine, reached as `newDesign()` and `families()`
   across the boundary. Six kinds, each from a preset held to its published
   figure. Heads are kept in whole groups over the key heads, with the depth
   taking up the size. A small model's head is tied when the vocabulary would
   be over a third of it. A new design trains at a pretraining length, not
   the reference's serving context. The fit search measures each candidate
   with the full analysis against the planner's headroom. The dialog previews
   the shape and one device's training footprint before anything is created,
   and Create can start the walkthrough. `scale` now keeps a stack's label
   ("x32") in step with its count.
2. **A drawing you can read when it opens.** A readable floor on the zoom a
   sheet opens at, and the plain names of M13 on the blocks' own summaries,
   which still read `D 48 · dh 16 · ffn 192`. *Done:* a sheet opens at 0.85 or
   better, whole when it fits and from its top when it does not, and `f` still
   fits all of it; the fit waits until React Flow holds the laid-out nodes,
   which is what made the first attempt open on an empty corner. Blocks say
   *32 heads × 128 · 8 kv* and *128,256 × 4,096*, and a phrase drops its least
   important parts rather than running into an ellipsis.
3. **Try a change and see what it did.** A baseline marked when a design is
   created or opened, and the difference from it shown in the readout, so
   "what if this were grouped-query" is one change and one glance. *Done:* the
   baseline was already kept; a strip under the parameter count now says what
   moved since it — a symbol by its description, a parameter by its label —
   and what that did to parameters, compute, cache and memory per GPU, measured
   by the engine's diff a moment after the last edit. Compare from here moves
   the baseline.

### M15 — Memory, measured · Phases 1–2 done
...
2. **Autocast, as a recipe.** A precision choice in the operating point beside
   ...
   Held to the measurement the same way. *Done:* `precision: "autocast"`
   follows each tensor's width through the graph, charges a bf16 copy per
   matrix multiply that reads an fp32 tensor, and holds every cast weight. It
   lands at 0.95–0.98× the measurement on both designs and both lengths, and
   the resting state's 4/4/8 split is within 1% part by part. It is in the
   operating point under More, in the CLI as `--precision`, and in the MCP
   tools.
