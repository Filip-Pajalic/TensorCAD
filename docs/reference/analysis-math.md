...
- **Where 6N/2N break**: (a) long context: with `N ≈ 12·L·D²` the attention share is `T/(6·D)` → 33% for Llama-3-8B at 8K, >100% at 32K; (b) MoE: use active `N`, but hardware FLOPs include padded/dropped tokens under capacity factors; (c) attention is `2·T·H·(d_qk + d_v)` per token before the causal half, the scores at the key width and the value product at the value width — they differ in latent attention (192 against 128 in DeepSeek-V3) and in differential attention (values twice the key width); MLA decode with weight absorption scores in latent space: `2·T·H·(d_c + d_rope)` per token, larger than GQA's `2·T·H·dh`; (d) MTP adds ≈ one extra layer + head per depth; (e) sliding-window layers: a causal window of `W` keeps `min(q + 1, W)` keys at position `q`, so replace the causal `T/2` by `W − W²/2T` once `T > W` (not `W/2`: the window already only looks back); a mask written as an expression is counted as the share of those it keeps, measured; sparse attention by `topk + W`; (f) SSM layers: linear in `T`; SSD scan ≈ `6·d_inner·N_s` FLOPs per token plus a chunk-quadratic term ≈ `4·d_inner·Q` for chunk `Q=256`, small vs the `in/out_proj` matmuls (**approximate; paper gives asymptotics not constants**); (g) elementwise ops (norm, RoPE, softmax, act) are ~1–3% of FLOPs but memory-bound: ignore for FLOPs, not for latency; (h) inference decode per token: `2·N_active + 4·L·T_ctx·H·dh`.
...
- **MLA**: `L·(d_c + d_rope)·bytes` (DeepSeek-V2 Table 1). V3: `61·576·2 = 70,272 B` ≈ 68.6 KiB/token vs 4 MiB uncompressed (57×). Engines that don't absorb weights cache decompressed K/V → MHA size; **the analysis exposes both**, as `bytesPerToken` and `bytesPerTokenDecompressed`. The decompressed figure is a key and a value per head plus the rotary part, which every head shares and so is held once: `L·(H·(d_nope + d_v) + d_rope)·bytes`. For V3 that is 3.82 MiB against 68.63 KiB, reproducing Table 1's 57x, and a test pins the ratio. A design with no latent attention reports the same number twice rather than a second figure that means nothing.
