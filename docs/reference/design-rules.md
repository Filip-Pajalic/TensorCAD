...
| `input-roles` | A port that reads an input for what it holds is not given it. A mask that keeps [packed documents](../explanation/packed-sequences.md) apart reads the documents, and a rotation or a position embedding that restarts at every document reads the positions. What reaches either, followed back through every stack it was handed into, has to be an input whose `role` says so. Token ids have the same shape and element type, so the two checks above pass them. |
